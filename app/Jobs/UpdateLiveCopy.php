...
class UpdateLiveCopy extends Job implements SelfHandling, ShouldQueue
{
  ...
  public function handle(Filesystem $fs, GitHubManager $gh, CacheRepository $cache)
  {
    ...
    $user = 'lu-j';
    $repo = 'oparl-json-schema';

    $files = $gh->repo()->contents()->show($user, $repo, '/src', 'HEAD');
    foreach ($files as $file)
    {
      if (ends_with($file['name'], '.md'))
      {
        // load a chapter file
        $data = $gh->repo()->contents()->download($user, $repo, $file['path']);
        $fs->put(LiveCopyRepository::CHAPTER_PATH.$file['name'], $data);
      }
    }

    // load images
    $images = $gh->repo()->contents()->show($user, $repo, '/src/images', 'HEAD');
    foreach ($images as $image)
    {
      // only load pngs
      if (ends_with($image['name'], '.png'))
      {
        $data = $gh->repo()->contents()->download($user, $repo, $image['path']);
